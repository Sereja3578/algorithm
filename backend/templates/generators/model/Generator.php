...
namespace backend\templates\generators\model;
...
class Generator extends \yii\gii\Generator
{
    ...
    public $tableName= '*';
    public $isGenerateOnlyBaseModel = true;
    public $isGenerateOnlyBaseQuery= true;
    public $modelClass;
    public $baseClass = 'common\components\ActiveRecord';
    public $queryNs = 'common\models\query';
    public $queryClass;
    public $queryBaseClass = 'common\components\ActiveQuery';
    ...
    public $baseClassNs = 'common\models\base\\';
    public $queryClassNs = 'common\models\query\base\\';
    ...
    public $modelClassReport;
    public $queryClassReport;
    public $nsReport = 'common\models\stats';
    public $queryNsReport = 'common\models\stats\query';
    public $baseClassNsReport = 'common\models\stats\base\\';
    public $queryClassNsReport = 'common\models\stats\query\base\\';
    public $queryBaseClassReport = 'common\components\ActiveQueryReport';
    public $baseClassReport = 'common\components\ActiveRecordReport';
    public $isGenerateOnlyBaseQueryReport = true;
    public $isGenerateOnlyBaseModelReport = true;


    /**
     * @inheritdoc
     */
    public function generateManyRelations()
    {
        $relations = [];
        $this->hasManyRelations = [];
        foreach ($this->generateRelations() as $tableName => $tableRelations) {
            $relations[$tableName] = [];
    ...
    {
        return 'Model Generator (Veselov)';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass', 'nsReport', 'queryNsReport', 'baseClassReport', 'queryClassReport', 'queryBaseClassReport', 'modelClassReport'], 'filter', 'filter' => 'trim'],
            [['ns', 'queryNs', 'nsReport', 'queryNsReport'], 'filter', 'filter' => function ($value) {
                return trim($value, '\\');
            }],
            [['db', 'ns', 'tableName', 'baseClass', 'queryNs', 'queryBaseClass', 'nsReport', 'queryNsReport', 'baseClassReport', 'queryBaseClassReport'], 'required'],
            [['db', 'modelClass', 'queryClass', 'modelClassReport', 'queryClassReport'], 'match', 'pattern' => '/^\w+$/', 'message' => 'Only word characters are allowed.'],
            [['ns', 'baseClass', 'queryNs', 'queryBaseClass', 'nsReport', 'queryNsReport', 'baseClassReport', 'queryBaseClassReport'], 'match', 'pattern' => '/^[\w\\\\]+$/', 'message' => 'Only word characters and backslashes are allowed.'],
            [['tableName'], 'match', 'pattern' => '/^(\w+\.)?([\w\*]+)$/', 'message' => 'Only word characters, and optionally an asterisk and/or a dot are allowed.'],
            [['db'], 'validateDb'],
            [['ns', 'queryNs', 'nsReport', 'queryNsReport'], 'validateNamespace'],
            [['tableName'], 'validateTableName'],
            [['modelClass', 'modelClassReport'], 'validateModelClass', 'skipOnEmpty' => false],
            [['baseClass', 'baseClassReport'], 'validateClass', 'params' => ['extends' => ActiveRecord::className()]],
            [['queryBaseClass', 'queryBaseClassReport'], 'validateClass', 'params' => ['extends' => ActiveQuery::className()]],
            [['generateLabelsFromComments', 'useTablePrefix', 'useSchemaName', 'generateQuery'], 'boolean'],
            [['enableI18N', 'isGenerateOnlyBaseModel', 'isGenerateOnlyBaseQuery', 'isGenerateOnlyBaseModelReport', 'isGenerateOnlyBaseQueryReport'], 'boolean'],
            [['generateRelations'], 'in', 'range' => [self::RELATIONS_NONE, self::RELATIONS_ALL, self::RELATIONS_ALL_INVERSE]],
            [['messageCategory'], 'validateMessageCategory', 'skipOnEmpty' => false],
            [['imagesDomain', 'imagesPath'], 'filter', 'filter' => 'trim'],
            [['imagesPath'], 'filter', 'filter' => function ($value) {
                return trim($value, '/');
            }],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            'ns' => 'Model Namespace',
            'nsReport' => 'Model Namespace',
            'db' => 'Database Connection ID',
            'tableName' => 'Table Name',
            'modelClass' => 'Model Class',
            'modelClassReport' => 'Model Class',
            'baseClass' => 'Model Base Class',
            'baseClassReport' => 'Model Base Class',
            'generateRelations' => 'Generate Relations',
            'isGenerateOnlyBaseModel' => 'Generate Only Base Model (without Model)',
            'isGenerateOnlyBaseModelReport' => 'Generate Only Base Model Report (without Model)',
            'isGenerateOnlyBaseQuery' => 'Generate Only Base Query (without Query)',
            'isGenerateOnlyBaseQueryReport' => 'Generate Only Base Query Report (without Query)',
            'generateLabelsFromComments' => 'Generate Labels from DB Comments',
            'generateQuery' => 'Generate ActiveQuery',
            'queryNs' => 'ActiveQuery Namespace',
            'queryNsReport' => 'ActiveQueryReport Namespace',
            'queryClass' => 'ActiveQuery Class',
            'queryClassReport' => 'ActiveQueryReport Class',
            'queryBaseClass' => 'ActiveQuery Base Class',
            'queryBaseClassReport' => 'ActiveQuery Base Report Class',
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            'isGenerateOnlyBaseModel' => 'This indicates whether the generator should generate only Base Model and not regenerate Model Classes.',
            'isGenerateOnlyBaseQuery' => 'This indicates whether the generator should generate only Base Query and not regenerate Query Classes.',
...
//            'modelIcon' => 'This is a model icon, e.g., <code>glyphicon-asterisk</code>',
    ...
    public function requiredTemplates()
    {
        ...
        return ['model.php', 'query.php', 'modelView.php', 'queryView.php'];
    }
     ...
     * @return string
     */
    public function statusConstants($tableSchema, $tableName)
    {
        $this->generateStatusAttributes($tableSchema, $tableName);

        foreach ($tableSchema->columns as $column) {
            if (in_array($column->name, /*array('status','email_approved', 'disabled')*/ $this->__statusAttributes[$tableName])) {
     ...
     * @return string
     */
    public function generateTimestampAttributes($tableSchema, $tableName)
    {
        if (!isset($this->__datetimeAttributes[$tableName])) {
            $this->__datetimeAttributes[$tableName] = [];
        }

        if (sizeof($this->__datetimeAttributes[$tableName])<1) {
            foreach ($tableSchema->columns as $column) {
                if (in_array(strtolower($column->dbType), array('timestamp', 'date', 'datetime'))) {
                    $this->__datetimeAttributes[$tableName][] = $column->name;
                }
            }
        }

        if (sizeof($this->__datetimeAttributes[$tableName])>0) {
            return "public static \$datetimeAttributes = ['".implode("','", $this->__datetimeAttributes[$tableName])."'];\n";
        } else {
            return "public static \$datetimeAttributes = [];\n";
        }
    }
     ...
     * @return string
     */
    public function generateStatusAttributes($tableSchema, $tableName)
    {
        if (!isset($this->__statusAttributes[$tableName])) {
            $this->__statusAttributes[$tableName] = [];
        }

        if (sizeof($this->__statusAttributes[$tableName])<1) {
            foreach ($tableSchema->columns as $column) {
                if (in_array($column->dbType, array('tinyint(1) unsigned'))) {
                    $this->__statusAttributes[$tableName][] = $column->name;
                }
            }
        }

        if (sizeof($this->__statusAttributes[$tableName])>0) {
            return "public static \$statusAttributes = ['".implode("','", $this->__statusAttributes[$tableName])."'];\n";
        } else {
            return "public static \$statusAttributes = [];\n";
        }
    }
     ...
     * @return string
     */
    public function generateRelationAttributes($tableSchema)
    {
        $relationAttributes = [];
        ...
        foreach ($tableSchema->columns as $column) {
            if ($this->existRelation($column)) {
                $relationAttributes[] = $column->name;
            }
        }

        return "protected \$relationAttributes = ['".implode("','", $relationAttributes)."'];\n";
    }
     ...
     * @return array
     */
    public function datetimeAttributes($tableSchema, $tableName)
    {
        static $attributes;
        if (!isset($attributes)) {
            $attributes = [];
        ...
        $this->generateTimestampAttributes($tableSchema, $tableName);

        if (!isset($attributes[$tableName])) {
            $attributes[$tableName] = [];
            foreach ($tableSchema->columns as $column) {
                if (in_array($column->name, $this->__datetimeAttributes[$tableName] /*array('created_at', 'updated_at', 'created', 'updated')*/)) {
     ...
     * @return string
     */
    public function modelNS($tableSchema, $tableName, $isTable = true)
    {
        $ns = ['Yii;'];
        $ns[] = ltrim(($isTable) ? $this->baseClass : $this->baseClassReport, '\\') .';';
        ...
        */
        ...
        if (isset($relations[$tableName]) && sizeof($relations[$tableName])>0) {
            foreach ($relations[$tableName] as $relation => $rs) {
                $ns[] = 'common\\models\\' . ltrim($rs[1], '\\') . ';';
            }
        }

        $datetimeAttributes = $this->datetimeAttributes($tableSchema, $tableName);
        if (!empty($datetimeAttributes)) {
            $ns[] = 'yii\\behaviors\\TimestampBehavior;';
            if (count($datetimeAttributes) < 2) {
                //$ns[] = 'yii\\db\\BaseActiveRecord;';
            }
        }
    ...
    public function getOtherKeys($tableSchema) {
        $pk = [];

        foreach ($this->getColumnNames($tableSchema) as $column) {
     ...
     * @return array|bool
     */
    public function isIdModelVirtual($attribute, $tableName = null, $tableSchema = null)
    {
        ...
        if (!empty($table)) {
            ...
            $modelClass = str_replace('\report', '', "{$this->ns}\\$class");

            $columns = $this->getClassColumns($tableSchema);
     ...
     */
    public function getClassColumns($tableSchema)
    {
    ...
    public function generateAllRelations()
    {
    ...
    public function getRelationKeys($relation, $tableName, $tableSchema)
    {
        $class = '\\common\\models\\' . $relation;

        if (class_exists($class)) {
            foreach ($class::getTableSchema()->foreignKeys as $foreignKey) {
                if ($foreignKey[0] == $tableName) {
     ...
     */
    public function generateNewMethods(\yii\db\TableSchema $tableSchema, $tableName)
    {
        $hasMany = false;

        $relations = $this->generateManyRelations();
        ...
        foreach ($relations as $tableName => $tableRelations) {
            ...
            foreach ($tableRelations as $relationName => $relation) {
                list ($code, $className, $hasMany) = $relation;
                $nsClassName = array_search($className, $this->classNames);
        ...
        {
            $model = new <?= $className ?>;
<?php foreach ($foreignKey as $key1 => $key2) : ?>
            $model-><?= $key1 ?> = $this-><?= $key2 ?>;
<?php endforeach; ?>
            return $model;
<?php
                                unset($foreignKey[0]);
    ...
    public function generateRules($tableSchema, $tableName)
    {
        ...
        foreach ($tableSchema->columns as $column) {
            ...
            if (!$column->allowNull && $column->defaultValue === null &&
                    (!in_array($column->name, array_merge(/*['created_at','update_at']*/$this->__datetimeAttributes[$tableName], $this->imageAttributes)))) {
                $types['required'][] = $column->name;
            }
            $this->generateStatusAttributes($tableSchema, $tableName);
            if (in_array($column->name, $this->__statusAttributes[$tableName] /*array('status', 'email_approved', 'enabled')*/)) {
                if (!$column->allowNull && !is_null($column->defaultValue)) {
                    $extra[] = "[['{$column->name}'], 'default', 'value' => '" . $column->defaultValue . "']";
                }
...
//                }
    ...
    */
    ...
    {
        $table instanceof \yii\db\TableSchema;
     ...
     * @return string
     */
    public function generateStringMessage($string, $tableName) {
        $this->messageCategory = Inflector::camel2id($tableName);
        return $this->generateString($string, [], false);
    }

    /**
     * @inheritdoc
     */
    public function generateString($string = '', $placeholders = [], $addSlashes = true)
    {
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            if (in_array($tableName, ['migration', 'auth_item', 'auth_item_child', 'auth_rule', 'auth_assignment'])) {
                continue;
            }

            $isTable = (strpos($tableName, '_report') === false) ? true : false;
            ...
            if ($isTable) {
                $files[] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/base/' . $modelClassName . 'Base.php',
                    $this->render('modelBase.php', $params)
                );

                if (!$this->isGenerateOnlyBaseModel) {
                    $files[] = new CodeFile(
                ...
                $files[] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', $this->nsReport)) . '/base/' . $modelClassName . 'Base.php',
                    $this->render('modelBaseView.php', $params)
                );

                if (!$this->isGenerateOnlyBaseModelReport) {
                    $files[] = new CodeFile(
                        Yii::getAlias('@' . str_replace('\\', '/', $this->nsReport)) . '/' . $modelClassName . '.php',
                        $this->render('modelView.php', $params)
                    );
                }
            }
            ...
            if ($queryClassName) {
                if ($isTable) {
                    ...
                    $files[] = new CodeFile(
                        Yii::getAlias('@' . str_replace('\\', '/', $this->queryNs)) . '/base/' . $queryClassName . 'Base.php',
                        $this->render('queryBase.php', $params)
                    );

                    if (!$this->isGenerateOnlyBaseQuery) {
                        $files[] = new CodeFile(
                    ...
                    $files[] = new CodeFile(
                        Yii::getAlias('@' . str_replace('\\', '/', $this->queryNsReport)) . '/base/' . $queryClassName . 'Base.php',
                        $this->render('queryBaseView.php', $params)
                    );

                    if (!$this->isGenerateOnlyBaseQueryReport) {
                        $files[] = new CodeFile(
                            Yii::getAlias('@' . str_replace('\\', '/', $this->queryNsReport)) . '/' . $queryClassName . '.php',
                            $this->render('queryView.php', $params)
                        );
                    }
                }
            }
